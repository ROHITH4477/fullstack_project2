...
export default function Index() {
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden">
        <img src={heroImg} alt="Beautiful Indian homestay at golden sunset" className="absolute inset-0 w-full h-full object-cover"/>
        ...
        <div className="relative z-10 text-center px-4 w-full max-w-6xl mx-auto pt-16">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold text-white mb-4 leading-tight animate-fade-in-up-delay-1" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {categories.map((cat) => (<Link key={cat.id} to={`/homestays?category=${cat.name}`} className="card-travel p-4 flex flex-col items-center text-center gap-2 hover:border-primary/30 hover:bg-primary/5 transition-all group">
      ...
      <section className="py-12 px-4 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {homestays.filter(h => h.isFeatured).map((homestay) => (<HomestayCard key={homestay.id} homestay={homestay}/>))}
        </div>
      </section>
      ...
      <section className="py-12 px-4 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {trendingDestinations.map((dest) => (<Link key={dest.name} to={`/homestays?location=${dest.name}`} className="group relative rounded-2xl overflow-hidden cursor-pointer">
              <img src={dest.image} alt={dest.name} className="w-full h-36 object-cover group-hover:scale-110 transition-transform duration-500" onError={(e) => {
          ...
          <div className="gradient-warm rounded-3xl p-8 md:p-12 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <button onClick={handleUseOfferCode} className="bg-white/20 backdrop-blur-sm border border-white/30 text-white font-mono font-bold px-6 py-3 rounded-xl tracking-widest hover:bg-white/25 transition-colors">
                  STAYFIRST
                </button>
              </div>
            </div>
      ...
      <section className="py-12 px-4 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {attractions.map((attr) => (<Link key={attr.id} to="/attractions" className="card-travel group overflow-hidden">
              <div className="relative overflow-hidden">
                <img src={attr.image} alt={attr.name} className="w-full h-44 object-cover group-hover:scale-105 transition-transform duration-500" onError={(e) => {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div>
              ...
              <div className="space-y-3">
                {restaurants.slice(0, 3).map((r) => (<Link key={r.id} to="/dining" className="card-travel p-4 flex gap-4 items-center group hover:border-primary/30">
            ...
            <div>
              ...
              <div className="space-y-3">
                {chefs.slice(0, 3).map((c) => (<Link key={c.id} to="/dining" className="card-travel p-4 flex gap-4 items-center group hover:border-primary/30">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {c.specialties.slice(0, 2).map((s) => (<span key={s} className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full font-medium">{s}</span>))}
                      </div>
                    </div>
      ...
      <section className="py-12 px-4 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {guides.map((guide) => (<Link key={guide.id} to="/guides" className="card-travel p-5 flex gap-4 items-start group hover:border-primary/30">
              <img src={guide.image} alt={guide.name} className="w-16 h-16 rounded-2xl object-cover flex-shrink-0 border-2 border-border group-hover:border-primary/40 transition-colors"/>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  {guide.specialties.slice(0, 2).map((s) => (<span key={s} className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full font-medium">{s}</span>))}
                </div>
      ...
      <section className="py-16 px-4 bg-muted/40">
        ...
        ].map(({ icon: Icon, title, desc }) => (<div key={title} className="bg-card rounded-2xl p-6 text-left border border-border hover:border-primary/30 transition-colors hover:shadow-md">
      ...
      <section className="py-16 px-4 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {reviews.map((rev) => (<div key={rev.id} className="card-travel p-6">
