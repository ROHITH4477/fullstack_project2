            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              <div className="card-travel p-5">
                ...
                <div className="grid grid-cols-2 gap-2 mb-5">
                  {PAYMENT_METHODS.map((m) => (<button key={m.id} onClick={() => setMethod(m.id)} className={`flex items-center gap-2 p-3 rounded-xl border-2 text-sm font-medium transition-all ${method === m.id ? "border-primary bg-primary/8 text-primary" : "border-border text-foreground hover:border-primary/40"}`}>
                      <m.icon className="h-4 w-4"/> {m.label}
                    </button>))}
                </div>

                {method === "card" && (<div className="space-y-3">
                    <div>
                ...
                {(method === "netbanking" || method === "wallet") && (<div className="grid grid-cols-3 gap-2">
                    {(method === "netbanking"
                ? ["SBI", "HDFC", "ICICI", "Axis", "Kotak", "PNB"]
                : ["Paytm", "PhonePe", "Amazon Pay", "Mobikwik"]).map((opt) => (<button key={opt} onClick={() => setSelectedProvider(opt)} className={`p-3 border rounded-xl text-sm transition-colors text-foreground font-medium ${selectedProvider === opt ? "border-primary bg-primary/8 text-primary" : "border-border hover:border-primary/50 hover:bg-primary/5"}`}>
                        {opt}
                      </button>))}
                  </div>)}
              </div>
