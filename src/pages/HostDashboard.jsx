...
import { Plus, Calendar, Star, DollarSign, Home, MessageSquare, CheckCircle, XCircle, Clock, Edit, Eye, TrendingUp } from "lucide-react";
export default function HostDashboard() {
    ...
    const updateBookingStatus = (requestId, status) => {
    ...
    const handleOverviewCardClick = (label) => {
    ...
    const handleOpenReply = (targetId) => {
      ...
      <div className="pt-16">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-8">
          {activeTab === "overview" && (<div className="space-y-8">
            ...
            ].map((stat) => (<button key={stat.label} onClick={() => handleOverviewCardClick(stat.label)} className="card-travel p-4 text-left w-full">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {bookingRequests.filter(b => b.status === "pending").map((req) => (<div key={req.id} className="card-travel p-4 flex items-center gap-4">
          ...
          {activeTab === "properties" && (<div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {properties.map((prop) => (<div key={prop.id} className="card-travel overflow-hidden">
                    <div className="relative">
                      <img src={prop.image} alt={prop.name} className="w-full h-48 object-cover" onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1568605114967-8130f3a36994?w=400&q=80"; }}/>
          ...
          {activeTab === "bookings" && (<div>
            ...
            ].map((filter) => (<button key={filter.id} onClick={() => setBookingStatusFilter(filter.id)} className={`text-xs px-3 py-1.5 rounded-full border ${bookingStatusFilter === filter.id ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:bg-muted"}`}>
                      {filter.label}
                    </button>))}
                </div>
              </div>
              <div className="space-y-4">
                {visibleBookingRequests.map((req) => (<div key={req.id} className="card-travel p-5 flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${req.status === "confirmed" ? "bg-green-100 text-green-700" :
          ...
          {activeTab === "earnings" && (<div>
            ...
            ].map((e) => (<div key={e.label} className="card-travel p-6 text-center">
            ...
            ].map((t, i) => (<div key={i} className="flex items-center justify-between py-3 border-b border-border last:border-0">
                      <div>
          ...
          {activeTab === "reviews" && (<div>
              ...
              <div className="space-y-3">
                {reviews.map((review) => (<div key={review.id} className="card-travel p-4">
                    ...
                    <div className="mt-3">
                      <button onClick={() => handleOpenReply(`review-${review.id}`)} className="btn-outline-primary text-xs py-1.5 px-3">
                        Reply
                      </button>
                      {activeReplyTarget === `review-${review.id}` && (<div className="mt-2 flex gap-2">
                          ...
                          <button onClick={() => handleSendReply(`review-${review.id}`, review.guest)} className="btn-primary text-xs py-1.5 px-3">
                            Send
                          </button>
                        </div>)}
                    </div>
          ...
          {activeTab === "profile" && (<div>
              ...
              <div className="card-travel p-6 max-w-xl">
                <div className="flex items-center gap-4 mb-4">
                  <img src={hostAvatar || "https://i.pravatar.cc/150"} alt="Host" className="w-16 h-16 rounded-2xl object-cover border border-border"/>
        ...
        {isAddingProperty && (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
