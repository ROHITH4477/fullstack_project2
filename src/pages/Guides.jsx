...
export default function Guides() {
    ...
    const handleContactGuide = (guideName, location) => {
        if (!isLoggedIn) {
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <div className="pt-16">
        <div className="gradient-nature text-white py-12 px-4 text-center">
          <h1 className="text-4xl font-bold mb-2" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Local Expert Guides</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...guides, ...guides].map((guide, i) => (<div key={i} className="card-travel p-6">
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {guide.specialties.map((s) => (<span key={s} className="bg-primary/10 text-primary text-xs px-2.5 py-1 rounded-full font-medium">{s}</span>))}
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => handleContactGuide(guide.name, guide.location)} className="btn-primary text-sm py-2 flex items-center gap-1.5">
