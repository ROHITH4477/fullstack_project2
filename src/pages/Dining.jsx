...
export default function Dining() {
    ...
    const handleBookChef = (chefName, location) => {
        if (!isLoggedIn) {
    ...
    const handleBookTable = (restaurantName, location) => {
        if (!isLoggedIn) {
            navigate("/auth?mode=login");
            return;
        }
      ...
      <div className="pt-16">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 pt-8">
          <div className="flex gap-2 mb-8">
            <button onClick={() => setTab("restaurants")} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all ${tab === "restaurants"
            ? "bg-primary text-primary-foreground shadow-md"
            : "bg-muted text-muted-foreground hover:bg-muted/80"}`}>
              <UtensilsCrossed className="h-4 w-4"/> Dine at a Restaurant
            </button>
            <button onClick={() => setTab("chefs")} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all ${tab === "chefs"
            ? "bg-primary text-primary-foreground shadow-md"
            : "bg-muted text-muted-foreground hover:bg-muted/80"}`}>
          ...
          {/* Restaurants */}
          {tab === "restaurants" && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-12">
              {restaurants.map((r) => (<div key={r.id} className="card-travel group overflow-hidden">
                  <div className="relative overflow-hidden">
                    <img src={r.image} alt={r.name} className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-500"/>
                    <div className="absolute top-3 left-3 flex gap-1.5">
                      {r.isVeg && (<span className="bg-secondary text-secondary-foreground text-xs font-semibold px-2 py-1 rounded-full flex items-center gap-1">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex flex-wrap gap-1 mb-3">
                      {r.cuisine.map((c) => (<span key={c} className="bg-primary/10 text-primary text-xs px-2 py-0.5 rounded-full font-medium">{c}</span>))}
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-border">
                      ...
                      <button onClick={() => handleBookTable(r.name, r.location)} className="btn-primary text-xs py-1.5 px-3">
                        Book Table
                      </button>
                    </div>
          ...
          {tab === "chefs" && (<div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-12">
                {chefs.map((chef) => (<div key={chef.id} className="card-travel p-6">
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      {chef.available && (<span className="bg-secondary/20 text-secondary text-xs font-semibold px-2 py-1 rounded-full">Available</span>)}
                    </div>
                    <p className="text-sm text-muted-foreground mb-4 line-clamp-2">{chef.about}</p>
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {chef.specialties.map((s) => (<span key={s} className="bg-primary/10 text-primary text-xs px-2.5 py-1 rounded-full font-medium">{s}</span>))}
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-border">
                      ...
                      <button onClick={() => handleBookChef(chef.name, chef.location)} className="btn-primary text-sm py-2 flex items-center gap-1.5">
