    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-16">
        <div className="max-w-5xl mx-auto px-4 md:px-6 py-8">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-foreground mb-8" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            <div className="lg:col-span-3 space-y-6">
              <div className="card-travel p-5 flex gap-4">
                <img src={homestay.image} alt={homestay.name} className="w-24 h-24 rounded-xl object-cover flex-shrink-0" onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1568605114967-8130f3a36994?w=200&q=80"; }} />
                <div>
              ...
              <div className="card-travel p-5">
                ...
                <div className="mt-4 flex items-center gap-2 text-sm text-muted-foreground bg-muted/30 rounded-xl p-3">
                  ...
                  <span><strong className="text-foreground">{nights} nights</strong> · INR {homestay.price.toLocaleString()}/night</span>
                </div>
              </div>
              ...
              <div className="card-travel p-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" className="input-search w-full" placeholder="Your first name" value={firstName} onChange={(e) => setFirstName(e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1.5">Last Name</label>
                    <input type="text" className="input-search w-full" placeholder="Your last name" value={lastName} onChange={(e) => setLastName(e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1.5">Email</label>
                    <input type="email" className="input-search w-full" placeholder="your@email.com" value={email} onChange={(e) => setEmail(e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1.5">Phone</label>
                    <input type="tel" className="input-search w-full" placeholder="+91 98765 43210" value={phone} onChange={(e) => setPhone(e.target.value)} />
                  </div>
                </div>
              </div>
              ...
              <div className="card-travel p-5">
                ...
                <textarea className="input-search w-full h-24 resize-none" placeholder="Eg: Early check-in, vegetarian food, baby cot required..." value={specialRequests} onChange={(e) => setSpecialRequests(e.target.value)} />
              </div>
              ...
              <button onClick={handleProceedToPayment} disabled={!agreeToTerms} className="btn-primary w-full flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                <CreditCard className="h-5 w-5" />
                Proceed to Payment · INR {total.toLocaleString()}
              </button>
            </div>
              ...
              <div className="card-travel p-6 sticky top-24">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between text-muted-foreground">
                    ...
                    <span>INR {cleaningFee.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-muted-foreground">
                    <span>StayVista service fee</span>
                    <span>INR {serviceFee.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-muted-foreground">
                    <span>GST (12%)</span>
