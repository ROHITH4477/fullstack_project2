...
export default function GuideDashboard() {
    ...
    const [replyTextByTarget, setReplyTextByTarget] = useState({});
    ...
    const handleBookingStatus = (requestId, status) => {
    ...
    const handleSaveItinerary = () => {
        ...
        setItineraries((prev) => prev.map((it) => (it.id === editingItineraryId
    ...
    const handleSendReply = (targetId, targetName) => {
      ...
      <div className="pt-16">
        <div className="bg-card border-b border-border px-4 md:px-6 py-6">
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center gap-4 mb-6">
              <img src={user?.avatar || myGuideProfile.image} alt="" className="w-14 h-14 rounded-2xl object-cover border-2 border-primary/30"/>
            ...
            <div className="flex gap-1 overflow-x-auto scrollbar-hide border-b border-border">
              {tabs.map((tab) => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${activeTab === tab.id ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-8">
          {activeTab === "overview" && (<div className="space-y-8">
            ...
            ].map((stat) => (<div key={stat.label} className="card-travel p-4 text-center">
              ...
              <div className="card-travel p-6">
                <div className="flex items-start gap-5">
                  <img src={myGuideProfile.image} alt={myGuideProfile.name} className="w-20 h-20 rounded-2xl object-cover"/>
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                      {myGuideProfile.specialties.map((s) => (<span key={s} className="bg-primary/10 text-primary text-xs px-3 py-1 rounded-full font-medium">{s}</span>))}
                    </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {bookingRequests.map((b) => (<div key={b.id} className="card-travel p-4 flex items-center gap-4">
          ...
          {activeTab === "itineraries" && (<div>
              ...
              <div className="space-y-4">
                {itineraries.map((it) => (<div key={it.id} className="card-travel p-5 flex items-center gap-4">
          ...
          {activeTab === "attractions" && (<div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {guideAttractions.map((attr) => (<div key={attr.id} className="card-travel flex gap-4 p-4">
                    <img src={attr.image} alt={attr.name} className="w-20 h-20 rounded-xl object-cover flex-shrink-0" onError={(e) => {
                    e.target.src = "https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=200&q=80";
                }}/>
                    <div>
          ...
          {activeTab === "bookings" && (<div>
              ...
              <div className="space-y-3">
                {bookingRequests.map((booking) => (<div key={booking.id} className="card-travel p-4 flex items-center justify-between gap-3">
                    <div>
          ...
          {activeTab === "reviews" && (<div>
              ...
              <div className="space-y-3">
                {reviews.map((review) => (<div key={review.id} className="card-travel p-4">
                    <div className="flex items-center justify-between">
                      <p className="font-semibold text-foreground">{review.tourist}</p>
                      <div className="flex items-center gap-1">
                        {Array.from({ length: review.rating }).map((_, i) => (<Star key={i} className="h-3.5 w-3.5 fill-accent text-accent"/>))}
                      </div>
                    </div>
                    <p className="text-sm text-muted-foreground mt-2">{review.text}</p>
                    <div className="mt-3">
                      <button onClick={() => setActiveReplyTarget(`review-${review.id}`)} className="btn-outline-primary text-xs py-1.5 px-3">
                        Reply
                      </button>
                      {activeReplyTarget === `review-${review.id}` && (<div className="mt-2 flex gap-2">
                          <input value={replyTextByTarget[`review-${review.id}`] || ""} onChange={(e) => setReplyTextByTarget((prev) => ({ ...prev, [`review-${review.id}`]: e.target.value }))} className="input-search w-full" placeholder="Type your reply"/>
                          <button onClick={() => handleSendReply(`review-${review.id}`, review.tourist)} className="btn-primary text-xs py-1.5 px-3">
                            Send
                          </button>
                        </div>)}
                    </div>
          ...
          {activeTab === "messages" && (<div>
              ...
              <div className="space-y-3">
                {messages.map((message) => (<div key={message.id} className="card-travel p-4 flex items-center justify-between gap-3">
                    <div>
                      <p className="font-semibold text-foreground text-sm">{message.from}</p>
                      <p className="text-sm text-muted-foreground">{message.text}</p>
                      {activeReplyTarget === `message-${message.id}` && (<div className="mt-2 flex gap-2">
                          <input value={replyTextByTarget[`message-${message.id}`] || ""} onChange={(e) => setReplyTextByTarget((prev) => ({ ...prev, [`message-${message.id}`]: e.target.value }))} className="input-search w-full" placeholder="Type your reply"/>
                          <button onClick={() => handleSendReply(`message-${message.id}`, message.from)} className="btn-primary text-xs py-1.5 px-3">
                            Send
                          </button>
                        </div>)}
                    </div>
          ...
          {activeTab === "profile" && (<div>
              ...
              <div className="card-travel p-6 max-w-xl">
                <div className="flex items-center gap-4 mb-4">
                  <img src={guideAvatar || myGuideProfile.image} alt="Guide" className="w-16 h-16 rounded-2xl object-cover border border-border"/>
        ...
        {isCreatingItinerary && (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <div className="bg-card border border-border rounded-2xl w-full max-w-lg p-5">
              ...
              <div className="space-y-3">
                <input value={itineraryTitle} onChange={(e) => setItineraryTitle(e.target.value)} className="input-search w-full" placeholder="Itinerary title"/>
                <div className="grid grid-cols-2 gap-3">
                  <input type="number" min={1} value={itineraryDays} onChange={(e) => setItineraryDays(Math.max(1, Number(e.target.value) || 1))} className="input-search w-full" placeholder="Days"/>
                  <input type="number" min={1} value={itineraryPlaces} onChange={(e) => setItineraryPlaces(Math.max(1, Number(e.target.value) || 1))} className="input-search w-full" placeholder="Places"/>
                </div>
              </div>
        ...
        {editingItineraryId && (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <div className="bg-card border border-border rounded-2xl w-full max-w-lg p-5">
        ...
        {isAddingAttraction && (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <div className="bg-card border border-border rounded-2xl w-full max-w-xl p-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                {newAttractionImage && (<img src={newAttractionImage} alt="Attraction preview" className="md:col-span-2 w-full h-40 rounded-xl object-cover border border-border"/>)}
              </div>
              <div className="flex justify-end gap-2 mt-4">
