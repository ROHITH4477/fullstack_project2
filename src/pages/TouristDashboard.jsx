...
import { Search, Heart, MapPin, Calendar, Bell, LayoutDashboard, Bookmark, History, Star, TrendingUp, User, Settings } from "lucide-react";
export default function TouristDashboard() {
    ...
    const handleViewBookingDetails = (tripId) => {
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <div className="pt-16">
        {/* Header */}
        <div className="bg-background/80 backdrop-blur-2xl border-b border-white/10 px-4 md:px-6 py-6">
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <img src={user?.avatar || "https://i.pravatar.cc/150"} alt={user?.name} className="w-12 h-12 rounded-2xl object-cover border-2 border-primary/30"/>
                <div>
              ...
              <div className="flex items-center gap-2 relative">
                <button onClick={() => setActiveTab("profile")} className="p-2.5 hover:bg-primary/12 rounded-2xl border border-transparent hover:border-primary/30 transition-colors" title="Profile Settings">
                  <Settings className="h-5 w-5 text-muted-foreground"/>
                </button>
                <button onClick={() => setNotificationsOpen((v) => !v)} className="relative p-2.5 hover:bg-primary/12 rounded-2xl border border-transparent hover:border-primary/30 transition-colors" title="Notifications">
                  <Bell className="h-5 w-5 text-muted-foreground"/>
                  <span className="absolute top-1 right-1 w-2 h-2 bg-destructive rounded-full"/>
                </button>
                {notificationsOpen && (<div className="absolute right-0 top-12 w-80 bg-card/95 backdrop-blur-xl border border-white/15 rounded-2xl shadow-lg z-20 p-3">
                    <p className="font-semibold text-sm text-foreground mb-2">Notifications</p>
                    <div className="space-y-2">
                      {notifications.map((item, i) => (<div key={i} className="text-xs text-muted-foreground bg-muted/50 rounded-xl p-2 border border-white/10">
                          {item}
                        </div>))}
                    </div>
                  </div>)}
            ...
            {/* Tabs */}
            <div className="flex gap-1.5 mt-6 overflow-x-auto scrollbar-hide">
              {tabs.map((tab) => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${activeTab === tab.id
                ? "border-primary text-primary bg-primary/10 rounded-t-2xl"
                : "border-transparent text-muted-foreground hover:text-foreground hover:bg-muted/60 rounded-t-2xl"}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-8">
          {activeTab === "discover" && (<div className="space-y-8">
              {/* Stats */}
            ...
            ].map((stat) => (<button key={stat.label} onClick={() => handleStatClick(stat.label)} className="card-travel p-4 text-center w-full">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {activeBookings.map((trip) => (<div key={trip.id} className="card-travel p-4 flex items-center gap-4">
                      <img src={trip.image} alt={trip.homestay} className="w-16 h-16 rounded-xl object-cover flex-shrink-0" onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1568605114967-8130f3a36994?w=100&q=80"; }}/>
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${trip.status === "Confirmed" ? "bg-primary/15 text-primary border border-primary/30" : "bg-accent/15 text-foreground border border-accent/30"}`}>
                          {trip.status}
                        </span>
                      </div>
                    </div>))}
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {homestays.slice(0, 3).map((h) => (<HomestayCard key={h.id} homestay={h}/>))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {attractions.slice(0, 3).map((attr) => (<div key={attr.id} className="card-travel overflow-hidden">
                      <img src={attr.image} alt={attr.name} className="w-full h-40 object-cover" onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=400&q=80"; }}/>
                      <div className="p-4">
          ...
          {activeTab === "attractions" && (<div>
              <h2 className="text-xl font-bold text-foreground mb-6 flex items-center gap-2">
                <MapPin className="h-5 w-5 text-primary"/> Tourist Attractions
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {attractions.map((attr) => (<div key={attr.id} className="card-travel overflow-hidden">
                    <img src={attr.image} alt={attr.name} className="w-full h-44 object-cover" onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=600&q=80"; }}/>
                    <div className="p-4">
          ...
          {activeTab === "wishlist" && (<div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {homestays.slice(0, 4).map((h) => (<HomestayCard key={h.id} homestay={h}/>))}
              </div>
            </div>)}
          ...
          {activeTab === "bookings" && (<div>
              ...
              <div className="space-y-4">
                {activeBookings.map((trip) => (<div key={trip.id} className="card-travel p-6">
                    <div className="flex items-start gap-4">
                      <img src={trip.image} alt={trip.homestay} className="w-20 h-20 rounded-xl object-cover" onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1568605114967-8130f3a36994?w=100&q=80"; }}/>
                      <div className="flex-1">
                        <div className="flex items-start justify-between">
                          ...
                          <span className={`text-xs font-semibold px-3 py-1 rounded-full ${trip.status === "Confirmed" ? "bg-primary/15 text-primary border border-primary/30" : "bg-accent/15 text-foreground border border-accent/30"}`}>
                            {trip.status}
                          </span>
                        </div>
                        ...
                        <div className="flex gap-3 mt-4">
                          <button onClick={() => handleViewBookingDetails(trip.id)} className="text-sm btn-outline-primary py-1.5">
                            View Details
                          </button>
                          <button onClick={() => handleCancelBooking(trip.id)} className="text-sm text-destructive border border-destructive/60 rounded-2xl px-4 py-1.5 hover:bg-destructive/10 transition-colors">
                            Cancel
                          </button>
                        </div>
                      </div>
                ...
                {activeBookings.length === 0 && (<div className="card-travel p-6 text-center text-muted-foreground">
          ...
          {activeTab === "profile" && (<div>
              ...
              <div className="card-travel p-6 max-w-xl">
                <div className="flex items-center gap-4 mb-4">
                  <img src={profileAvatar || "https://i.pravatar.cc/150"} alt="Profile" className="w-16 h-16 rounded-2xl object-cover border border-border"/>
                ...
                <div className="space-y-3">
                  <input value={profileName} onChange={(e) => setProfileName(e.target.value)} className="input-search w-full" placeholder="Name"/>
                  <input value={profileEmail} onChange={(e) => setProfileEmail(e.target.value)} className="input-search w-full" placeholder="Email"/>
                </div>
          ...
          {activeTab === "history" && (<div>
                ...
                { name: "Rishikesh River View", location: "Rishikesh", date: "Aug 2024", amount: 1998, rating: 5 },
            ].map((h, i) => (<div key={i} className="card-travel p-4 flex items-center gap-4">
                    ...
                    <div className="text-right">
                      ...
                      <div className="flex items-center gap-0.5 justify-end">
                        {Array.from({ length: h.rating }).map((_, i) => (<Star key={i} className="h-3 w-3 fill-accent text-accent"/>))}
                      </div>
                    </div>
                  </div>))}
              </div>
        ...
        {selectedTrip && (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <div className="bg-card/95 backdrop-blur-xl border border-white/15 rounded-3xl w-full max-w-md p-5 shadow-xl">
