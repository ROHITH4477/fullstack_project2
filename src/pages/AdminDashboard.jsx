...
export default function AdminDashboard() {
    ...
    const handleUserDecision = (userId, status) => {
    ...
    const handleOverviewStatClick = (label) => {
        if (label === "Tourists") {
            setActiveTab("users");
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <div className="pt-16">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-8">
          {activeTab === "overview" && (<div className="space-y-8">
            ...
            ].map((stat) => (<button key={stat.label} onClick={() => handleOverviewStatClick(stat.label)} className="card-travel p-4 text-left w-full">
          ...
          {activeTab === "users" && (<div>
            ...
            ].map((filter) => (<button key={filter.id} onClick={() => setUserRoleFilter(filter.id)} className={`text-xs px-3 py-1.5 rounded-full border ${userRoleFilter === filter.id ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:bg-muted"}`}>
                      {filter.label}
                    </button>))}
                </div>
              </div>
              <div className="space-y-3">
                {visibleUsers.map((entry) => (<div key={entry.id} className="card-travel p-4 flex items-center justify-between gap-3">
                    <div>
                    ...
                    <div className="text-right">
                      ...
                      {entry.status === "Pending" && (<div className="flex gap-2 mt-2 justify-end">
                          <button onClick={() => handleUserDecision(entry.id, "Active")} className="text-xs bg-green-100 text-green-700 px-2.5 py-1 rounded-lg hover:bg-green-200 transition-colors">
                            Approve
                          </button>
                          <button onClick={() => handleUserDecision(entry.id, "Rejected")} className="text-xs bg-red-100 text-destructive px-2.5 py-1 rounded-lg hover:bg-red-200 transition-colors">
                            Reject
                          </button>
                        </div>)}
                    </div>
          ...
          {(activeTab === "interviews" || activeTab === "appointments") && (<div>
            ...
            ].map((filter) => (<button key={filter.id} onClick={() => setCandidateFilter(filter.id)} className={`text-xs px-3 py-1.5 rounded-full border ${candidateFilter === filter.id ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:bg-muted"}`}>
                      {filter.label}
                    </button>))}
                </div>
              </div>
              <div className="space-y-4">
                {visibleCandidates.map((candidate) => (<div key={candidate.id} className="card-travel p-5">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <a href={candidate.resumeFile} target="_blank" rel="noreferrer" className="text-xs text-primary font-medium hover:underline mt-1 inline-block">
                          View Resume File
                        </a>
                        {candidate.interviewDate && <p className="text-xs text-primary mt-1">Interview: {candidate.interviewDate}</p>}
                      </div>
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full capitalize ${candidate.status === "appointed"
                    ...
                    <div className="flex flex-wrap gap-2 mt-4">
                      {(candidate.status === "applied" || candidate.status === "rejected") && (<button onClick={() => scheduleInterview(candidate.id)} className="btn-outline-primary text-xs py-1.5 px-3">Schedule Call</button>)}
                      {candidate.status === "interview_scheduled" && (<button onClick={() => markInterviewDone(candidate.id)} className="btn-outline-primary text-xs py-1.5 px-3">Mark Interview Done</button>)}
                      {(candidate.status === "interviewed" || candidate.status === "interview_scheduled") && (<button onClick={() => setPendingDecision({ candidateId: candidate.id, action: "appoint" })} className="btn-primary text-xs py-1.5 px-3">Appoint</button>)}
                      {candidate.status !== "appointed" && (<button onClick={() => setPendingDecision({ candidateId: candidate.id, action: "reject" })} className="text-xs text-destructive border border-destructive rounded-xl px-3 py-1.5 hover:bg-destructive/10 transition-colors">Reject</button>)}
                    </div>
                  </div>))}
                {visibleCandidates.length === 0 && (<div className="card-travel p-6 text-center text-muted-foreground">
          ...
          {activeTab === "profile" && (<div>
              ...
              <div className="card-travel p-6 max-w-xl">
                <div className="flex items-center gap-4 mb-4">
                  <img src={adminAvatar || "https://i.pravatar.cc/150"} alt="Admin" className="w-16 h-16 rounded-2xl object-cover border border-border"/>
      ...
      {pendingDecision && decisionCandidate && (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-2xl w-full max-w-lg p-5">
            ...
            <div className="card-travel p-3 mb-3">
              ...
              <a href={decisionCandidate.resumeFile} target="_blank" rel="noreferrer" className="text-xs text-primary font-medium hover:underline mt-2 inline-block">
                Open Resume File
              </a>
            </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={confirmDecision} className={`${pendingDecision.action === "appoint" ? "btn-primary" : "text-destructive border border-destructive hover:bg-destructive/10"} text-sm py-1.5 px-4 rounded-xl transition-colors`}>
