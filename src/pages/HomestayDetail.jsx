...
import { Star, MapPin, Heart, Share2, ChevronLeft, ChevronRight, Wifi, Car, Utensils, Waves, Shield, Users, BedDouble, CheckCircle, X as XIcon } from "lucide-react";
const amenityIcons = {
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <div className="pt-16">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 mb-8">
          <div className="grid grid-cols-4 grid-rows-2 gap-2 rounded-3xl overflow-hidden h-80 md:h-[450px] border border-white/10 shadow-[0_24px_55px_-38px_hsl(var(--primary)/0.95)]">
            <div className="col-span-2 row-span-2 relative overflow-hidden group cursor-pointer" onClick={() => setImgIdx(0)}>
              <img src={images[0]} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1568605114967-8130f3a36994?w=800&q=80"; }}/>
            </div>
            {images.slice(1, 4).map((img, i) => (<div key={i} className="relative overflow-hidden group cursor-pointer" onClick={() => setImgIdx(i + 1)}>
                <img src={img} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1568605114967-8130f3a36994?w=400&q=80"; }}/>
                {i === 2 && (<div className="absolute inset-0 bg-foreground/40 flex items-center justify-center">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 pb-16">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div>
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div>
                    ...
                    <h1 className="text-2xl md:text-3xl font-bold text-foreground mt-2" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
                      {homestay.name}
                    </h1>
                  </div>
                  <div className="flex gap-2 flex-shrink-0">
                    <button onClick={() => setWishlisted(!wishlisted)} className="p-2.5 border border-white/15 bg-card/65 backdrop-blur-xl rounded-2xl hover:bg-primary/12 hover:border-primary/30 transition-colors">
                      <Heart className={`h-5 w-5 ${wishlisted ? "fill-red-500 text-red-500" : "text-muted-foreground"}`}/>
                    </button>
                    <button className="p-2.5 border border-white/15 bg-card/65 backdrop-blur-xl rounded-2xl hover:bg-primary/12 hover:border-primary/30 transition-colors">
              ...
              {/* Host */}
              <div className="flex items-center gap-4 p-4 bg-card/70 backdrop-blur-xl rounded-3xl border border-white/12">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {homestay.amenities.map((amenity) => (<div key={amenity} className="flex items-center gap-3 p-3 bg-card/65 backdrop-blur-xl rounded-2xl border border-white/12">
        ...
        ].map(({ rule, allowed }) => (<div key={rule} className="flex items-center gap-2 text-sm text-foreground">
                      {allowed
            ...
            { user: "Rohit Kumar", avatar: "https://i.pravatar.cc/50?img=65", rating: 5, text: "One of the best experiences of my life. The food was homemade and delicious!", date: "Dec 2024" },
        ].map((rev) => (<div key={rev.user} className="p-4 bg-card/65 backdrop-blur-xl border border-white/12 rounded-2xl">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="ml-auto flex">
                          {Array.from({ length: rev.rating }).map((_, i) => (<Star key={i} className="h-3.5 w-3.5 fill-accent text-accent"/>))}
                        </div>
                      </div>
            ...
            <div className="lg:col-span-1">
              <div className="card-travel p-6 sticky top-24">
                ...
                <div className="space-y-3 mb-4">
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input type="date" className="input-search w-full text-sm" value={checkIn} min={new Date().toISOString().split("T")[0]} onChange={(e) => setCheckIn(e.target.value)}/>
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1">Check Out</label>
                      <input type="date" className="input-search w-full text-sm" value={checkOut} min={checkIn} onChange={(e) => setCheckOut(e.target.value)}/>
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1">Guests</label>
                    <select className="input-search w-full text-sm" value={guests} onChange={(e) => setGuests(Number(e.target.value))}>
                      {Array.from({ length: homestay.guests }, (_, i) => i + 1).map((n) => (<option key={n} value={n}>{n} {n === 1 ? "guest" : "guests"}</option>))}
                    </select>
                  </div>
                </div>

                {nights > 0 && (<div className="border-t border-border/80 pt-4 mb-4 space-y-2">
                ...
                <Link to={`/booking/${homestay.id}?checkin=${checkIn}&checkout=${checkOut}&guests=${guests}`} className="btn-primary w-full text-center block">
